...
const ICONS = {
  ...
  diamond: <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 3h12l4 6-10 13L2 9Z"/><path d="M11 3 8 9l4 13 4-13-3-6"/><line x1="2" y1="9" x2="22" y2="9"/></svg>,
  chair: <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 11h14v4H5zM3 15h18v2H3zM7 17v4M17 17v4M8 11V5a4 4 0 0 1 8 0v6"/></svg>,
  star: <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
};
...
export default async function HomePage({ params }) {
  ...
  return (
    <>
      ...
      <section className="relative flex flex-col md:flex-row min-h-[100svh]">
        ...
        <div className="order-2 md:order-1 md:w-[44%] lg:w-[40%] bg-[#f5ede8] flex flex-col justify-center px-6 sm:px-10 md:px-10 lg:px-12 py-10 md:py-24 relative z-10">
          <Reveal>
            ...
            <Link
              href={`/${lang}#contacts`}
              className="inline-flex w-fit items-center gap-3 bg-[#c9a898] hover:bg-[#b8967a] text-white text-[10px] tracking-[0.25em] sm:tracking-[0.3em] md:tracking-[0.4em] uppercase px-6 md:px-8 py-3.5 md:py-4 transition-colors"
            >
              ЗАПИСАТЬСЯ ОНЛАЙН
            </Link>
            ...
            <Reveal key={i} delay={i * 60}>
              <div className="flex gap-3 items-start px-4 sm:px-6 lg:px-4 py-5 md:py-7 lg:py-8">
                ...
                <div>
                  ...
                  <div className="text-[10px] sm:text-xs text-[#9a8878] hidden sm:block">{f.sub}</div>
                </div>
              </div>
            </Reveal>
          ))}
      ...
      {/* ── SERVICES ──────────────────────────────────── */}
      <section id="services" className="py-16 md:py-28 bg-[#f5ede8] scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          ...
          <Reveal>
            ...
            <h2 className="font-display font-light text-4xl md:text-6xl text-[#1a1714] mb-10 md:mb-12">НАШИ УСЛУГИ</h2>
      ...
      {/* ── GALLERY ───────────────────────────────────── */}
      <section className="py-16 md:py-24 bg-[#ede3da]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <Reveal>
            <div className="flex items-end justify-between mb-8 md:mb-10">
              <div>
                <p className="text-[10px] tracking-[0.45em] uppercase text-[#b8976a] mb-3">Атмосфера</p>
                <h2 className="font-display font-light text-3xl md:text-5xl text-[#1a1714]">НАША СТУДИЯ</h2>
              </div>
            </div>
          </Reveal>
          <Reveal>
      ...
      {/* ── TEAM PREVIEW ──────────────────────────────── */}
      <section id="team" className="py-16 md:py-28 bg-[#f5ede8] scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <Reveal>
            <div className="flex items-end justify-between mb-10 md:mb-12 flex-wrap gap-4">
              <div>
                ...
                <h2 className="font-display font-light text-3xl md:text-5xl text-[#1a1714]">НАШИ МАСТЕРА</h2>
              </div>
              <Link
                href={`/${lang}/team`}
                className="text-[10px] tracking-[0.35em] uppercase text-[#b8976a] hover:text-[#1a1714] transition-colors inline-flex items-center gap-2"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-7">
            {teamMembers.map((m, i) => (
              <Reveal key={m._id || i} delay={i * 80}>
                <div>
                  <div className="aspect-[3/4] bg-[#ede3da] overflow-hidden mb-3 md:mb-4 rounded-sm">
                    <SmartImage src={m.photoUrl} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                  </div>
                  <p className="font-display text-sm md:text-lg text-[#1a1714]">{m.name}</p>
                  <p className="text-[9px] sm:text-[10px] tracking-wider uppercase text-[#b8976a] mt-1">{typeof m.role === "object" ? m.role.ru : m.role}</p>
                  <p className="text-[10px] sm:text-xs text-[#9a8878] mt-0.5 hidden sm:block">{typeof m.experience === "object" ? m.experience.ru : m.experience}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* ── REVIEWS ───────────────────────────────────── */}
      <section className="py-16 md:py-24 bg-[#1a1714]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <Reveal>
            <div className="text-center mb-10 md:mb-14">
              <p className="text-[10px] tracking-[0.45em] uppercase text-[#b8976a] mb-4">Что говорят клиенты</p>
              <h2 className="font-display font-light text-3xl md:text-5xl text-white">ОТЗЫВЫ</h2>
            </div>
          </Reveal>
          <Reveal>
            <Carousel>
              {reviews.map((r, ri) => (
                <div key={r._id || ri} className="snap-start shrink-0 w-[85vw] sm:w-80 md:w-96 bg-[#221d19] border border-[#2d2520] p-6 sm:p-8 flex flex-col">
      ...
      {/* ── CTA BAND ──────────────────────────────────── */}
      <section className="py-16 md:py-28 bg-[#ede3da]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <Reveal>
            <p className="text-[10px] tracking-[0.45em] uppercase text-[#b8976a] mb-5">Начните сейчас</p>
            <h2 className="font-display font-light text-3xl sm:text-4xl md:text-6xl text-[#1a1714] mb-6">
              ЗАПИШИТЕСЬ<br/>НА КОНСУЛЬТАЦИЮ
            </h2>
            <p className="text-[#6b5f50] text-sm mb-8 md:mb-10 max-w-md mx-auto leading-relaxed">
              Мы подберём процедуры, которые подойдут именно вам. Первая консультация — бесплатно.
            </p>
            <Link
              href={`/${lang}#contacts`}
              className="inline-flex items-center gap-3 bg-[#1a1714] hover:bg-[#2d2520] text-white text-[10px] tracking-[0.25em] sm:tracking-[0.4em] uppercase px-8 sm:px-10 py-4 transition-colors"
            >
              ЗАПИСАТЬСЯ ОНЛАЙН
            </Link>
          </Reveal>
      ...
      {/* ── CONTACTS ──────────────────────────────────── */}
      <section id="contacts" className="py-16 md:py-28 bg-[#f5ede8] scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-2 gap-12 lg:gap-16">
          <Reveal>
            ...
            <h2 className="font-display font-light text-3xl md:text-5xl text-[#1a1714] mb-8 md:mb-10">КОНТАКТЫ</h2>
