...
export default function ServicesTab({ services, lang = "ru" }) {
  ...
  function handleBook(serviceName) {
    // Save to localStorage so ContactForm can pre-fill
    if (typeof window !== "undefined") {
      localStorage.setItem("lumiere_selected_service", serviceName);
    }
  }
        ...
        {items.map((item, i) => {
          ...
          return (
            <div key={i}>
              <div
                onClick={() => toggle(i)}
                className={`flex items-center justify-between py-4 border-b border-[#ede3da] cursor-pointer -mx-1 px-1 transition-colors ${isSelected ? "bg-[#faf6f2]" : "hover:bg-[#faf6f2]"}`}
              >
                <span className={`text-sm transition-colors ${isSelected ? "text-[#b8976a]" : "text-[#1a1714]"}`}>
                  {item.name}
                </span>
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <span className={`w-5 h-5 rounded-full border flex items-center justify-center text-xs leading-none transition-all duration-200 ${
                    isSelected
                      ? "bg-[#b8976a] border-[#b8976a] text-white"
                      : "border-[#b8976a] text-[#b8976a]"
                  }`}>
                    {isSelected ? "−" : "+"}
                  </span>
                </div>
              ...
              {isSelected && (
                <div className="flex items-center justify-between gap-4 py-3 px-1 bg-[#faf6f2] border-b border-[#ede3da]">
                  <span className="text-xs text-[#9a8878] truncate">
